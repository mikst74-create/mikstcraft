...
void main() {
    ...
    gl_Position = mvp * vec4(vec3(gsFrameCoo.xy, 0.0)+pos, 1.0);
    gs_out.surfacePos = vec2(gsTextureCoo.xy);
    EmitVertex();

    gl_Position = mvp * vec4(vec3(gsFrameCoo.zy, 0.0)+pos, 1.0);
    gs_out.surfacePos = vec2(gsTextureCoo.zy);
    EmitVertex();

    gl_Position = mvp * vec4(vec3(gsFrameCoo.xw, 0.0)+pos, 1.0);
    gs_out.surfacePos = vec2(gsTextureCoo.xw);
    EmitVertex();

    gl_Position = mvp * vec4(vec3(gsFrameCoo.zw, 0.0)+pos, 1.0);
